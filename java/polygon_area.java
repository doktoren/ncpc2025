...
class polygon_area {
    ...
    static void testRectangle() {
        Point[] rect = {new Point(0.0, 0.0), new Point(5.0, 0.0), new Point(5.0, 3.0), new Point(0.0, 3.0)};
        assert polygonArea(rect) == 15.0;

        Point[] rectCw = {new Point(0.0, 0.0), new Point(0.0, 3.0), new Point(5.0, 3.0), new Point(5.0, 0.0)};
        assert polygonArea(rectCw) == 15.0;
    }

    static void testDiamond() {
        Point[] diamond = {new Point(0.0, 2.0), new Point(3.0, 0.0), new Point(0.0, -2.0), new Point(-3.0, 0.0)};
        double area = polygonArea(diamond);
        assert area == 12.0;
    }

    static void testSignedArea() {
        Point[] ccw = {new Point(0.0, 0.0), new Point(1.0, 0.0), new Point(1.0, 1.0), new Point(0.0, 1.0)};
        assert polygonSignedArea(ccw) == 1.0;
        assert !isClockwise(ccw);

        Point[] cw = {new Point(0.0, 0.0), new Point(0.0, 1.0), new Point(1.0, 1.0), new Point(1.0, 0.0)};
