...
public:
    GrundyEngine(std::function<std::vector<T>(const T&)> move_function)
        : moves(move_function) {}
    ...
    int grundy(const T& state) const {
        ...
        for (const auto& next_state : next_states) {
            nimbers.push_back(grundy(next_state));
        }
